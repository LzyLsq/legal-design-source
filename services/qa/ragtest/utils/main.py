...
# 设置常量和配置
INPUT_DIR = str(config.path_from(__file__, os.pardir, "inputs", "artifacts"))
...
async def setup_llm_and_embedder():
    ...
    llm = ChatOpenAI(
        ...
        # api_key=os.getenv("GRAPHRAG_CHAT_API_KEY", ""),  # API Key
    ...
    text_embedder = OpenAIEmbedding(
        # # 调用gpt
        # api_base="https://api.wlai.vip/v1",  # 请求的API服务地址
        # api_key=os.getenv("GRAPHRAG_EMBEDDING_API_KEY", ""),  # API Key
...
if __name__ == "__main__":
    ...
    uvicorn.run(app, host=config.setting("SERVICE_HOST", "127.0.0.1"), port=int(config.setting("SERVICE_PORT", str(PORT))))
